...
function ensurePerformanceScript(html, prefix) {
  const tag = `<script defer src="${prefix}gh-performance.js?v=3"></script>`;
...
function ensureHeaderCssOrder(html, prefix) {
  const headerHref = `${prefix}site-header.css?v=32`;
...
function syncFile(rel) {
  ...
  const enhHref = `${prefix}gh-site-enhancements.css?v=21`;
