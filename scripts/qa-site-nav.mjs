...
const EXPECT_CSS = 'site-header.css?v=33';
...
for (const rel of collectHtml(ROOT)) {
  ...
  if (html.includes('<footer dir=')) {
    ...
    if (!/gh-site-enhancements\.css\?v=23/.test(html)) {
      issues.push(`${rel}: missing versioned footer CSS (gh-site-enhancements.css?v=23)`);
    }
  }
  ...
  if (/services\/[^/]+-en\.html$/.test(rel) && html.split('\n').length < 500) {
    issues.push(`${rel}: thin EN service page (${html.split('\n').length} lines) — expected rich content`);
  }
}
