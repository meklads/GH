...
const css = `
  :root {
    --pl-gold: #C9A84C;
    --pl-gold-soft: rgba(201,168,76,.16);
    --pl-ink: #0A0A0A;
    --pl-muted: #5a5a5a;
    --pl-soft: #7a7a7a;
    --pl-bg: #FAFAF8;
    ...
    --pl-line-strong: rgba(10,10,10,.14);
    --pl-measure: 38rem;
    --pl-pad-x: clamp(20px, 4vw, 40px);
    --pl-section: clamp(80px, 11vw, 128px);
    --pl-radius: 2px;
  }
  ...
  @media (max-width: 720px) { body.pl-lp { padding-bottom: 80px; } }
  .pl-lp * { box-sizing: border-box; }
  .pl-wrap { width: min(1080px, calc(100% - (var(--pl-pad-x) * 2))); margin-inline: auto; }
  .pl-eyebrow {
    display: block; color: var(--pl-gold); font-size: 11px; font-weight: 700;
    letter-spacing: .22em; text-transform: uppercase; margin-bottom: 16px;
    direction: ltr; unicode-bidi: isolate; text-align: start;
  }
  .pl-h2 {
    font-size: clamp(26px, 3.6vw, 40px); font-weight: 800; line-height: 1.28;
    margin: 0 0 16px; letter-spacing: -0.02em; max-width: 18em;
  }
  .pl-lead {
    font-size: clamp(16px, 1.45vw, 18px); font-weight: 400; line-height: 1.8;
    color: var(--pl-muted); margin: 0; max-width: var(--pl-measure);
  }
  .pl-btn {
    ...
    padding: 15px 26px; border: none; cursor: pointer; border-radius: var(--pl-radius);
    transition: transform .22s ease, background .22s ease, box-shadow .22s ease;
  }
  .pl-btn:hover { transform: translateY(-1px); background: #d4b45a; box-shadow: 0 10px 24px rgba(201,168,76,.28); }
  .pl-btn:active { transform: translateY(0); }
  .pl-section { padding: var(--pl-section) 0; }
  ...
  .pl-cta-row { margin-top: 32px; }

  /* Social proof */
  .pl-proof {
    padding: 22px 0 26px; border-bottom: 1px solid var(--pl-line); background: #fff;
  }
  .pl-proof-title {
    text-align: center; font-size: 13px; font-weight: 600; color: var(--pl-soft);
    margin: 0 0 16px; line-height: 1.5; letter-spacing: .02em;
  }
  .pl-logos {
    display: flex; flex-wrap: wrap; align-items: center; justify-content: center;
    gap: 14px 32px; margin: 0 auto 18px; max-width: 860px;
  }
  .pl-logos img {
    height: 32px; width: auto; max-width: 110px; object-fit: contain;
    opacity: .62; filter: grayscale(1); transition: opacity .2s;
  }
  .pl-logos img:hover { opacity: .95; }
  .pl-logo-text {
    font-size: 13px; font-weight: 800; color: rgba(10,10,10,.48); letter-spacing: .04em;
  }
  .pl-stats {
    display: flex; flex-wrap: wrap; justify-content: center; gap: 0;
    padding-top: 16px; border-top: 1px solid var(--pl-line); max-width: 680px; margin: 0 auto;
  }
  .pl-stats span {
    font-size: 12px; font-weight: 600; color: var(--pl-ink); text-align: center;
    padding: 0 18px; position: relative;
  }
  .pl-stats span + span::before {
    content: ''; position: absolute; inset-inline-start: 0; top: 50%;
    width: 1px; height: 12px; margin-top: -6px; background: var(--pl-line-strong);
  }
  .pl-stats strong { color: var(--pl-gold); font-weight: 800; }

  /* Self-assessment */
  .pl-assess { list-style: none; padding: 0; margin: 28px 0 0; display: grid; gap: 0; border-top: 1px solid var(--pl-line); }
  .pl-assess li {
    display: flex; gap: 14px; align-items: flex-start;
    padding: 18px 4px; border-bottom: 1px solid var(--pl-line);
    font-size: 16px; line-height: 1.55; font-weight: 600;
  }
  .pl-assess .q {
    color: var(--pl-gold); font-weight: 800; flex-shrink: 0; width: 1.4em;
    font-variant-numeric: tabular-nums;
  }
  .pl-assess-note {
    margin-top: 24px; padding: 22px 24px; background: var(--pl-ink); color: #fff;
    font-size: 16px; font-weight: 700; line-height: 1.6; border-inline-start: 3px solid var(--pl-gold);
  }
  ...
  .pl-wa-float {
    ...
    background: #fff; border: 1px solid var(--pl-gold); color: var(--pl-ink);
    text-decoration: none; box-shadow: 0 4px 16px rgba(0,0,0,.08);
    transition: transform .2s, background .2s, color .2s;
  }
  .pl-wa-float:hover { background: var(--pl-gold); }
  .pl-wa-float svg { width: 22px; height: 22px; fill: currentColor; }
  @media (max-width: 720px) {
    .pl-wa-float { bottom: calc(86px + env(safe-area-inset-bottom)); width: 46px; height: 46px; }
  }

  /* Sticky mobile CTA */
  .pl-sticky-cta {
    display: none; position: fixed; inset-inline: 0; bottom: 0; z-index: 950;
    padding: 10px var(--pl-pad-x) calc(10px + env(safe-area-inset-bottom));
    background: rgba(250,250,248,.97); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
    border-top: 1px solid var(--pl-line);
  }
  .pl-sticky-cta .pl-btn { width: 100%; padding: 15px 16px; font-size: 14px; }
  ...
  .pl-lp-header-inner {
    width: min(1120px, calc(100% - (var(--pl-pad-x) * 2))); margin: 0 auto;
    display: flex; align-items: center; justify-content: space-between; gap: 14px;
    min-height: 72px;
  }
  .pl-lp-logo img { height: 52px; width: auto; display: block; }
  .pl-lp-actions { display: flex; align-items: center; gap: 8px; flex-wrap: nowrap; justify-content: flex-end; }
  .pl-lp-actions .pl-btn { padding: 11px 16px; font-size: 12px; }
  .pl-lp-icon {
    width: 40px; height: 40px; display: inline-flex; align-items: center; justify-content: center;
    border: 1px solid rgba(255,255,255,.14); color: rgba(255,255,255,.85); text-decoration: none;
    background: rgba(255,255,255,.04); border-radius: var(--pl-radius);
    transition: border-color .2s, color .2s, background .2s;
  }
  .pl-lp-icon:hover { border-color: var(--pl-gold); color: var(--pl-gold); background: rgba(201,168,76,.08); }
  @media (max-width: 720px) {
    .pl-lp-logo img { height: 42px; }
    .pl-lp-header-inner { min-height: 64px; }
  ...
  @media (max-width: 900px) { .pl-grid-2 { grid-template-columns: 1fr; gap: 28px; } }
  .pl-media-frame {
    overflow: hidden; background: #111; aspect-ratio: 4/3; border-radius: var(--pl-radius);
  }
  .pl-media-frame img, .pl-media-frame video { width: 100%; height: 100%; object-fit: cover; display: block; }

  .pl-pain-list { list-style: none; padding: 0; margin: 28px 0 0; display: grid; gap: 0; border-top: 1px solid var(--pl-line); }
  .pl-pain-list li {
    padding: 18px 2px; border-bottom: 1px solid var(--pl-line);
    font-size: 16px; line-height: 1.65; color: var(--pl-ink);
  }
  .pl-pain-list strong { font-weight: 800; }
  .pl-punch {
    margin-top: 32px; font-size: clamp(20px, 2.3vw, 28px); font-weight: 800; line-height: 1.4;
    color: var(--pl-ink); max-width: 18em; border-inline-start: 3px solid var(--pl-gold);
  ...
  .pl-trust-card {
    background: #fff; padding: 28px 24px; min-height: 150px;
    border-top: 2px solid transparent; transition: border-color .2s;
  }
  .pl-trust-card:hover { border-top-color: var(--pl-gold); }
  .pl-trust-card h3 { margin: 0 0 10px; font-size: 17px; font-weight: 800; line-height: 1.35; }
  .pl-trust-card p { margin: 0; color: var(--pl-muted); line-height: 1.7; font-size: 14px; }

  .pl-product-box {
    background: var(--pl-ink); color: #fff; padding: clamp(40px, 6vw, 64px);
    border-inline-start: 3px solid var(--pl-gold);
  }
  .pl-product-box .pl-eyebrow { color: var(--pl-gold); }
  .pl-product-box .pl-h2 { color: #fff; max-width: 16em; }
  .pl-product-box .pl-lead { color: rgba(255,255,255,.78); }

  .pl-day-list { list-style: none; padding: 0; margin: 28px 0 0; display: grid; gap: 0; border-top: 1px solid var(--pl-line); }
  .pl-day-list li {
    display: flex; gap: 12px; align-items: flex-start;
    padding: 16px 2px; border-bottom: 1px solid var(--pl-line);
    font-size: 16px; font-weight: 600; line-height: 1.5;
  }
  .pl-day-list .ck { color: var(--pl-gold); flex-shrink: 0; font-size: 20px; margin-top: 1px; }

  /* Timeline */
  .pl-timeline { margin-top: 40px; display: grid; gap: 0; max-width: 560px; }
  .pl-tl-step { display: grid; grid-template-columns: 22px 1fr; gap: 18px; }
  .pl-tl-rail { display: flex; flex-direction: column; align-items: center; }
  .pl-tl-dot {
    width: 11px; height: 11px; border-radius: 50%; background: var(--pl-gold);
    box-shadow: 0 0 0 4px var(--pl-gold-soft); margin-top: 7px; flex-shrink: 0;
  }
  .pl-tl-line { flex: 1; width: 1px; background: linear-gradient(180deg, rgba(201,168,76,.55), rgba(201,168,76,.12)); min-height: 36px; }
  .pl-tl-body { padding-bottom: 32px; }
  .pl-tl-step:last-child .pl-tl-body { padding-bottom: 0; }
  ...
  .pl-story-meta {
    color: var(--pl-gold); font-size: 11px; font-weight: 800; letter-spacing: .16em;
    text-transform: uppercase; margin-bottom: 14px;
  }
  .pl-story h3 {
    font-size: clamp(22px, 2.8vw, 30px); margin: 0 0 28px; line-height: 1.35; font-weight: 800;
    max-width: 22em;
  }
  .pl-story-flow {
    display: grid; grid-template-columns: 1fr 1fr; gap: 0 40px;
    border-top: 1px solid var(--pl-line); padding-top: 8px;
  }
  @media (max-width: 800px) { .pl-story-flow { grid-template-columns: 1fr; } }
  .pl-story-flow > div {
    padding: 20px 0; border-bottom: 1px solid var(--pl-line);
  }
  .pl-story-flow strong {
    display: block; color: var(--pl-gold); font-size: 12px; margin-bottom: 8px;
    letter-spacing: .1em; font-weight: 800;
  }
  .pl-story-flow p { margin: 0; color: var(--pl-muted); line-height: 1.75; font-size: 15px; }
  .pl-story-media {
    display: grid; grid-template-columns: 1.35fr 1fr; gap: 10px; margin-top: 28px;
  }
  ...
  .pl-strip-item:hover img { transform: scale(1.03); opacity: 1; }
  .pl-strip-cap {
  ...
  /* FAQ */
  .pl-faq { display: grid; gap: 0; margin-top: 28px; border-top: 1px solid var(--pl-line); }
  .pl-faq details {
    background: transparent; border-bottom: 1px solid var(--pl-line); padding: 0;
  }
  .pl-faq summary {
  ...
  .pl-faq summary::after {
    content: '+'; color: var(--pl-gold); font-size: 20px; font-weight: 400; flex-shrink: 0;
  }
  .pl-faq details[open] summary::after { content: '−'; }
  .pl-faq details[open] summary { color: var(--pl-ink); padding-bottom: 10px; }
  .pl-faq p {
    margin: 0; padding: 0 0 20px; color: var(--pl-muted); line-height: 1.75; font-size: 15px;
  ...
  .pl-form-section .pl-lead { color: rgba(255,255,255,.72); }
  .pl-form {
    margin-top: 36px; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px;
    background: rgba(255,255,255,.03); border: 1px solid rgba(255,255,255,.1);
    padding: clamp(24px, 3.5vw, 36px); border-radius: var(--pl-radius);
  }
  ...
  .pl-form input, .pl-form select {
    width: 100%; padding: 15px 14px; border: 1px solid rgba(255,255,255,.16);
    background: rgba(0,0,0,.4); color: #fff; font-family: inherit; font-size: 15px;
    border-radius: var(--pl-radius); transition: border-color .2s;
  }
  ...
  .pl-form .pl-btn { width: 100%; padding: 17px; font-size: 15px; margin-top: 4px; }
  .pl-form-note { margin: 14px 0 0; font-size: 12px; color: rgba(255,255,255,.5); line-height: 1.65; }
  .pl-form-feedback {
  ...
  .pl-lp-footer {
    padding: 26px 0; border-top: 1px solid var(--pl-line); background: var(--pl-bg);
  }
  .pl-lp-footer-inner {
    width: min(1080px, calc(100% - (var(--pl-pad-x) * 2))); margin: 0 auto;
    display: flex; flex-wrap: wrap; gap: 10px 24px; justify-content: space-between; align-items: center;
    color: var(--pl-soft); font-size: 12px;
  }
  .pl-lp-footer a { color: var(--pl-ink); text-decoration: none; font-weight: 700; }
  .pl-lp-footer a:hover { color: var(--pl-gold); }
  /* Site page: normal mega-nav chrome on light body */
  body.pl-lp.pl-site .pl-hero {
    padding-top: calc(var(--gh-header-height, 88px) + 24px);
  }
  @media (max-width: 980px) {
    body.pl-lp.pl-site .pl-hero {
      padding-top: calc(var(--gh-header-height-mobile, 72px) + 16px);
    }
  }
  body.pl-lp.pl-site .pl-wa-float { display: none; }
`;

function btn(href = '#lead-form', label = CTA, extraClass = '') {
  return `<a class="pl-btn ${extraClass}" href="${href}" data-cta="book-session">${label}</a>`;
}


function adsHeader() {
  return `<header class="pl-lp-header" id="pl-lp-header">
  <div class="pl-lp-header-inner">
    ...
    <div class="pl-lp-actions">
      ...
      <a class="pl-btn" href="#lead-form" data-cta="book-session"><span class="pl-btn-label">${CTA}</span><span class="material-symbols-outlined" style="font-size:18px">arrow_downward</span></a>
    </div>
  </div>
</header>`;
}
...
function adsFooter() {
  return `<footer class="pl-lp-footer">
  <div class="pl-lp-footer-inner">
    <span>© Graphics House · ProjectLaunch™</span>
    <span>
      <a href="tel:${PHONE}">${PHONE_DISPLAY}</a>
      ·
      <a href="${WA}" target="_blank" rel="noopener" data-cta="whatsapp">واتساب</a>
      ·
...
function adsFloatWa() {
  return `<a class="pl-wa-float" href="${WA}" target="_blank" rel="noopener" aria-label="واتساب" title="واتساب" data-cta="whatsapp">
...
const MAIN = `<main id="main-content">
  ...
  <section class="pl-hero" id="hero" aria-label="ProjectLaunch">
    ...
    <div class="pl-hero-copy pl-reveal">
      <span class="pl-hero-brand">ProjectLaunch™</span>
...
<div class="pl-sticky-cta" id="plStickyCta">
  <a class="pl-btn" href="#lead-form" data-cta="book-session">${CTA}</a>
</div>
